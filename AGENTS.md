...
- Installer: per-user NSIS (no UAC → silent auto-update via
  electron-updater; channel selector in nav drawer). Unsigned for now
  (SmartScreen "More info → Run anyway" documented in README).
- Clean-machine test: `scripts/sandbox/installer-test.wsb` (Windows
  Sandbox; also asserts the fresh-machine zero-logs empty state) + Docker
  servercore harness. EQ install dir: env → registry → drive-sweep
