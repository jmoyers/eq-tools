...
- Repo: `C:\Users\jmoye\eq-tools` (public: github.com/jmoyers/eq-tools).
...
- Backticked EQ names (`Innoruuk\`s Chosen`) break inline `node -e` — use
  temp script files.
- Errors harness: main+renderer errors append to
  `%APPDATA%\eq-tools\errors.log` AND dev stdout, grep `[eq-tools:error]`
  (source tags: `main:uncaughtException`, `renderer:ErrorBoundary`, …).
  ErrorBoundary prevents blank windows. Check it first when anything's weird.
...
- CI (`.github/workflows/build.yml`): push to main → prerelease on the
  ...
  until 6 `AZURE_*` repo secrets exist (account `jmoyers-eqtools`, endpoint
...
- Publish: `publish: github jmoyers/eq-tools`; artifacts
  `eq-tools-Setup-<version>.exe` + `.blockmap` (differential updates) +
...
1. **Local self-test** (any dev machine, no elevation): run the Setup exe
   `/S` → assert files under `%LOCALAPPDATA%\Programs\eq-tools`, Start-menu
   shortcut, branded exe metadata; launch (expect the single-instance lock
   to focus a running dev app — that's a PASS); `Uninstall*.exe /S` →
   assert cleanup, appData preserved. Cheap smoke for every dist build.
