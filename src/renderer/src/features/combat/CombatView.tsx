  ...
  return (
    <Paper variant="outlined" sx={{ p: 1, height: 190, display: 'flex', flexDirection: 'column' }}>
      ...
      >
        ...
        {lines.map((l, i) => (
          <Box key={i} sx={{ display: 'flex', gap: 1, color: ROLE_COLOR[l.role] ?? 'text.primary', lineHeight: 1.5 }}>
...
export default function CombatView(): JSX.Element {
  const { snap, combinePets, setCombinePets, showUnparsed, setShowUnparsed, selection, setSelection } = useCombat()
  ...
  return (
    <Stack spacing={1.5} sx={{ height: '100%' }}>
      <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap" useFlexGap>
        <Select size="small" value={selection} onChange={(e) => setSelection(e.target.value)} sx={{ minWidth: 240 }}>
      ...
      <Alert severity="info" sx={{ py: 0 }}>
        ...
        still tracked. Encounters group staggered adds and freeze DPS when a fight ends; overall resets on zone. Toggle
        <strong> show unparsed</strong> to reveal any damage-shaped line the parser didn&apos;t understand.
      </Alert>
    </Stack>
  )
}
